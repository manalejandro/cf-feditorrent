...
export default function NotificationsPage() {
  ...
  const typeLabels: Record<string, string> = {
    follow: locale === "es" ? "te siguió" : "followed you",
    follow_accept: locale === "es" ? "aceptó tu solicitud" : "accepted your follow request",
    follow_reject: locale === "es" ? "rechazó tu solicitud" : "rejected your follow request",
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 py-12 w-full">
        <h1 className="text-3xl font-bold mb-6">{d.nav.notifications}</h1>
        ...
        {loading ? (
          ...
          <div className="bg-card border border-border rounded-xl p-8 text-center">
            <p className="text-muted">{locale === "es" ? "Aún no hay notificaciones." : "No notifications yet."}</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted mt-0.5">
                      {new Date(n.createdAt).toLocaleDateString(locale === "es" ? "es-ES" : "en-US")}
                    </p>
                  </div>
